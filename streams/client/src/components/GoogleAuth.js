...
class GoogleAuth extends React.Component {
  ...
  componentDidMount (){
    window.gapi.load('client:auth2', () =>{
      window.gapi.client.init({
        ...
        this.onAuthChange(this.auth.isSignedIn.get()); //llama a lo mismo
  ...
  onAuthChange=(isSignedIn)=>{
    if (isSignedIn){
      this.props.signIn(this.auth.currentUser.get().getId()); //quieres pasar el id del usuario que hace signIn
    }else{
      this.props.signOut();
    }
  };
